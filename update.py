...
def normalize_story(item: dict) -> dict:
    """Conservatively normalize an English story without rewriting source facts."""
    item = dict(item)
    title = clean(item.get("title", ""))
    title = re.sub(
        r"^(David Ornstein|Fabrizio Romano|Gianluca Di Marzio|Simon Stone)\s*(?:confirms?|reports?|reveals?|says?|:|-|—)\s*",
        "",
        title,
        flags=re.I,
    ).strip()
    ...
    item.setdefault("clubs", [])
    item.setdefault("club", "")
    item["language"] = "en"
    return item


def entity_key(item: dict) -> str:
    """Create a stable deduplication key without treating reporters as players."""
    entity = normalized(item.get("entity", ""))
    if entity in {
        "david ornstein", "fabrizio romano", "gianluca di marzio",
        "simon stone", "florian plettenberg", "ben jacobs",
        "matteo moretto", "sacha tavolieri",
    }:
        entity = ""
