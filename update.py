...
UA = "Mozilla/5.0 Fotbollsbriefen/8.1"
NOW = dt.datetime.now(dt.timezone.utc)
CUTOFF = NOW - dt.timedelta(days=CFG.get("max_age_days", 7))
MAX_ITEMS = min(int(CFG.get("max_items", 120)), 50)
...
def read_existing() -> list[dict]:
    """Version 8.0 rebuilds the feed from current sources only."""
    return []
...
SOURCE_PREFIX = re.compile(
    r"^(David Ornstein|Fabrizio Romano|Gianluca Di Marzio|Simon Stone)\s*:\s*",
    re.I,
)

JOURNALIST_PHRASES = re.compile(
    r"\b(enligt|rapporterar|uppger|skriver|säger|reports?|according to)\s+"
    r"(David Ornstein|Fabrizio Romano|Gianluca Di Marzio|Simon Stone)\b",
    re.I,
)

TRANSFER_VERBS = re.compile(
    r"\b(värvar|värvat|köper|lånar in|lånar ut|ansluter|klar för|"
    r"signs|signed|joins|joined|loan move|agrees deal)\b",
    re.I,
)

COACH_VERBS = re.compile(
    r"\b(blir ny tränare|blir ny förbundstränare|utses till tränare|"
    r"appointed manager|appointed coach|becomes manager|becomes coach)\b",
    re.I,
)


def strip_source_prefix(title: str) -> str:
    return SOURCE_PREFIX.sub("", title).strip()


def is_journalist(name: str) -> bool:
    normalized_name = re.sub(r"\s+", " ", name).strip()
    return normalized_name in KNOWN_JOURNALISTS


def is_known_coach(name: str) -> bool:
    normalized_name = re.sub(r"\s+", " ", name).strip()
    return normalized_name in KNOWN_COACHES


def person_candidates(text: str) -> list[str]:
    ...
    for candidate in candidates:
        ...
        if candidate in seen or is_journalist(candidate):
            continue
        seen.add(candidate)
...
def infer_subject(title: str, summary: str, categories: list[str]) -> tuple[str, str]:
    clean_title = strip_source_prefix(editorial_title(title))
    combined = f"{clean_title}. {summary}"

    # Explicit journalist attributions can never become the subject.
    combined = JOURNALIST_PHRASES.sub("", combined)

    # Role phrases are strongest.
    for pattern, role in PLAYER_ROLE_PATTERNS:
        match = re.search(pattern, combined, re.I)
        if match:
            candidate = match.group(1).strip()
            if not is_journalist(candidate):
                return candidate, role

    candidates = person_candidates(clean_title)

    # For coaching stories, prefer a known coach or the first non-journalist name.
    if "Tränare" in categories or COACH_VERBS.search(clean_title):
        for candidate in candidates:
            if is_known_coach(candidate):
                return candidate, "tränare"
        if candidates:
            return candidates[0], "tränare"

    # For transfers, prefer names next to transfer verbs and avoid clubs.
    if "Transfer" in categories or TRANSFER_VERBS.search(clean_title):
        for candidate in candidates:
            if any(club.lower() in candidate.lower() for club, _ in CLUB_PATTERNS):
                continue
            return candidate, "spelare"
...
def derive_story_type(categories: list[str], title: str) -> str:
    if "Tränare" in categories:
        return "coach"
    if "Transfer" in categories:
        return "transfer"
    if re.search(r"\b(skada|injury|operation|surgery)\b", title, re.I):
        return "injury"
    if re.search(r"\b(kontrakt|förlänger|extension|renew)\b", title, re.I):
        return "contract"
    return "general"


def factual_swedish_brief(item: dict, person: str, role: str, club_a: str, club_b: str) -> str:
    title = item.get("title", "").strip()
    categories = item.get("category", [])
    story_type = derive_story_type(categories, title)
    status = item.get("status", "")

    if story_type == "transfer":
        if person and club_a and club_b:
            verb = "har gjort klart med" if status == "Officiellt" else "uppges arbeta med en värvning av"
            return (
                f"{club_a} {verb} {person} från {club_b}. "
                f"Uppgifterna bygger på rapportering från huvudkällan"
                + (" och flera bekräftande källor." if item.get("confirmed_by") else ".")
            )
        if person and club_a:
            return (
                f"{club_a} uppges vara intresserat av {person}. "
                "Kontakter eller förhandlingar pågår, men affären är ännu inte officiellt klar."
            )
        if person:
            return (
                f"{person} är aktuell för en övergång. "
                "Fler detaljer väntas när de berörda klubbarna har kommit längre i processen."
            )

    if story_type == "coach":
        if person and club_a:
            if status == "Officiellt":
                return f"{person} har utsetts till ny tränare för {club_a}. Klubben har bekräftat beskedet."
            return (
                f"{person} är aktuell för tränaruppdraget i {club_a}. "
    ...
    if story_type == "injury" and person:
        return (
            f"{person} berörs av en skadeuppdatering. "
            "Omfattningen och den väntade frånvaron framgår av huvudkällans rapportering."
        )

    if story_type == "contract" and person:
        return (
            f"{person} är aktuell för ett nytt eller förlängt avtal. "
            "De slutliga villkoren bekräftas av klubben eller huvudkällan."
        )

    return ""


PLAYER_ROLE_PATTERNS = [
    (r"\bspanske försvararen\s+([A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+(?:\s+[A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+){0,3})", "försvarare"),
    (r"\bengelska försvararen\s+([A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+(?:\s+[A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+){0,3})", "försvarare"),
    (r"\bmålvakten\s+([A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+(?:\s+[A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+){0,3})", "målvakt"),
    (r"\bmittfältaren\s+([A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+(?:\s+[A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+){0,3})", "mittfältare"),
    (r"\banfallaren\s+([A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+(?:\s+[A-ZÅÄÖÉÜ][\wÅÄÖåäöÉéÜü'-]+){0,3})", "anfallare"),
]


def extract_person_and_role(
    title: str,
    summary: str = "",
    categories: list[str] | None = None,
) -> tuple[str, str]:
    return infer_subject(title, summary, categories or [])


def extract_transfer_clubs(text: str) -> tuple[str, str]:
    clubs = []
    for club, pattern in CLUB_PATTERNS:
        if re.search(pattern, text, re.I):
            clubs.append(club)
...
def editorial_source_filter(sources: list[dict]) -> list[dict]:
    strong = []
    seen = set()
    for source in sources:
        name = clean_source_name(source.get("name", "").strip())
        if not name or name in WEAK_CONFIRMERS:
            continue
        if name in OFFICIAL_NAMES or name in SOURCE_RANK:
            if name not in seen:
                seen.add(name)
                strong.append({**source, "name": name})
    return strong


def build_swedish_story(item: dict) -> dict:
    item = dict(item)
    title = strip_source_prefix(editorial_title(item.get("title", "")))
    raw_summary = clean(item.get("summary", ""))
    categories = item.get("category", [])

    # Normalize recurring machine-translation errors.
    title = re.sub(
        r"^(.+?) värvar Spanien för att försvara (.+?) från (.+)$",
        lambda m: f"{m.group(1)} värvar {m.group(2)} från {m.group(3)}",
        title,
        flags=re.I,
    )
    title = re.sub(
        r"^Englands försvarare Charles kommer till Manchester City från Chelsea$",
        "Manchester City värvar Niamh Charles från Chelsea",
        title,
        flags=re.I,
    )
    title = re.sub(r"\bMan United\b", "Manchester United", title, flags=re.I)
    title = re.sub(
        r"\|\s*Nyheter\s*\|\s*Officiell webbplats.*$",
        "",
        title,
        flags=re.I,
    ).strip()

    person, role = extract_person_and_role(title, raw_summary, categories)
    club_a, club_b = extract_transfer_clubs(f"{title}. {raw_summary}")

    sources = editorial_source_filter(item.get("sources", []))
    main, confirmed = strongest_source(sources, item)
    item["sources"] = sources
    item["main_source"] = main
    item["confirmed_by"] = confirmed
    if main:
        item["status"] = source_status(main["name"], 1 + len(confirmed))

    item["title"] = title
    item["entity"] = person
    item["club"] = club_a or item.get("club", "")
    item["entity_type"] = "coaches" if role == "tränare" else "players"

    generated = factual_swedish_brief(item, person, role, club_a, club_b)

    if generated:
        item["summary"] = generated
    else:
        candidates = []
        for sentence in re.split(r"(?<=[.!?])\s+", raw_summary):
            sentence = sentence.strip()
            if len(sentence) < 35 or not looks_swedish(sentence):
                continue
            if normalized(sentence) == normalized(title):
                continue
            sentence = re.sub(
                r"\b(Reuters|BBC Sport|Sky Sports|Official Website|Official Site)\b\s*$",
                "",
                sentence,
                flags=re.I,
            ).strip()
        ...
        item["summary"] = (
            ...
            or f"{title}. Uppgifterna bygger på rapportering från huvudkällan."
        )[:900]
...
def strongest_source(
    ...
    sources = editorial_source_filter(sources)
    if not sources:
        return None, []
    ...
    main = ranked[0]
    confirmed = ranked[1:5]
    return main, confirmed


def editorial_synthesis(item: dict) -> dict:
    item = dict(item)
    item["title"] = editorial_title(item.get("title", ""))
    item["summary"] = editorial_summary(item.get("summary", ""), item["title"])
    return build_swedish_story(item)
...
def translate_item(item: dict) -> dict:
    item = dict(item)
    item["title"] = google_translate(item.get("title", ""))
    item["summary"] = google_translate(item.get("summary", ""))
    item["language"] = "sv"
    return build_swedish_story(item)
...
selected = [
    build_swedish_story(item) for item in selected
    if item.get("title")
    and not re.search(
        r"\b(officiella hemsida|official website|club website|på bilder|i bilder|"
        r"presskonferens|women|women's|damlag|vm-rivaliteten|world cup quarter-finals|"
        r"academy|under-21|under-18)\b",
        item.get("title", ""),
        re.I,
    )
]
selected = [
    ...
    and not (
        item.get("entity") in KNOWN_JOURNALISTS
        or re.search(
            r"\b(värva|värvar|köpa|låna)\s+"
            r"(David Ornstein|Fabrizio Romano|Gianluca Di Marzio|Simon Stone)\b",
            item.get("title", ""),
            re.I,
        )
