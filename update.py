...
UA = "Mozilla/5.0 Fotbollsbriefen/8.0"
...
def extract_person_and_role(text: str) -> tuple[str, str]:
    for pattern, role in PLAYER_ROLE_PATTERNS:
        match = re.search(pattern, text, re.I)
        if match:
            return match.group(1).strip(), role
    return identify_person(text), ""
...
def build_swedish_story(item: dict) -> dict:
    ...
    person, role = extract_person_and_role(text)
    club_a, club_b = extract_transfer_clubs(text)
    categories = item.get("category", [])

    # Correct common translation errors in headlines.
    title = re.sub(
    ...
    title = re.sub(
        r"^(.+?) värvar den engelska försvararen Charles från Chelsea$",
        r"\1 värvar Niamh Charles från Chelsea",
        title,
        flags=re.I,
    )
    title = re.sub(
    ...
    title = re.sub(
        r"\bChelsea-stjärnan\b",
        person if person else "spelaren",
        title,
        flags=re.I,
    )
    title = re.sub(
    ...
    # Build a new Swedish brief from extracted facts, rather than translating
    # the source excerpt word-for-word.
    summary = ""
    if "Transfer" in categories:
        if person and club_a and club_b:
            summary = (
                f"{club_a} arbetar med en övergång för {person} från {club_b}. "
                "Uppgifterna stöds av flera etablerade källor. "
                "Affären är inte helt klar förrän klubbarna har bekräftat samtliga delar."
            )
        elif person and club_a:
            summary = (
                f"{club_a} uppges vilja värva {person}. "
                "Kontakter eller förhandlingar pågår enligt huvudkällan. "
                "Ett officiellt besked väntas om parterna når en slutlig överenskommelse."
            )
        elif person:
            summary = (
                f"{person} är aktuell för en övergång. "
                "Flera etablerade källor följer utvecklingen, men affären är ännu inte officiellt klar."
            )
    elif "Tränare" in categories:
        if person and club_a:
            summary = (
                f"{person} är aktuell för tränaruppdraget i {club_a}. "
                "De berörda parterna väntas lämna fler besked när processen är slutförd."
            )
        elif person:
            summary = (
                f"{person} är aktuell för ett nytt tränaruppdrag. "
                "Ett formellt besked väntas från de berörda parterna."
            )

    if not summary:
        # Keep concrete Swedish sentences only; otherwise use a neutral brief.
        candidates = []
        for sentence in re.split(r"(?<=[.!?])\s+", raw_summary):
            sentence = sentence.strip()
            if len(sentence) < 35 or not looks_swedish(sentence):
                continue
            if re.search(r"\b(Reuters|BBC Sport|Sky Sports|Official Website|Official Site)\b$", sentence, re.I):
                sentence = re.sub(r"\b(Reuters|BBC Sport|Sky Sports|Official Website|Official Site)\b$", "", sentence, flags=re.I).strip()
        ...
        if len(summary) < 70:
            summary = f"{title}. Uppgifterna bygger på rapportering från de angivna källorna."
