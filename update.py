...
USER_AGENT = "Mozilla/5.0 Fotbollsbriefen/Final"
...
OFFICIAL_SOURCES = {
    ...
    "Real Madrid", "Atlético Madrid", "Inter", "AC Milan", "Juventus", "Napoli",
...
def parse_feed(xml_bytes: bytes, source: dict[str, Any], cutoff: dt.datetime) -> list[dict[str, Any]]:
    ...
    for node in root.findall(".//item"):
        ...
        score = source_score(publisher, int(source.get("priority", 50)))
        if NEWS_SIGNAL_RE.search(combined):
            score += 25
        if len(clean_text(raw_description)) < 45:
            score -= 15
        if score < 80:
            continue
