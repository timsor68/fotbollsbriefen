...
def make_editorial_summary(item: dict) -> str:
    ...
    if len(summary) >= 90 and normalized(summary) != normalized(title):
        return summary[:900]

    if "Transfer" in categories:
        if person and club:
            return (
                f"{club} arbetar med en övergång för {person}. "
                "Uppgifterna stöds av en eller flera etablerade källor, men "
                "alla delar av affären är ännu inte officiellt bekräftade."
            )
        if person:
            return (
                f"{person} är aktuell för en övergång. "
                "Förhandlingar eller kontakter uppges pågå och fler besked väntas."
            )

    if "Tränare" in categories:
        if person and club:
            return (
                f"{person} kopplas till tränaruppdraget i {club}. "
                "Klubben väntas lämna fler besked när processen är slutförd."
            )
        if person:
            return (
                f"{person} är aktuell för ett nytt tränaruppdrag. "
                "Uppgifterna väntas klarna när berörda parter kommenterar situationen."
            )

    if title:
        return f"{title}. Uppgifterna bygger på rapportering från de angivna källorna."
    return summary
