...
SOURCE_RANK = {
    "Reuters": 100,
    "BBC Sport": 99,
    "Simon Stone / BBC Sport": 99,
    "David Ornstein / The Athletic": 98,
    "The Athletic": 97,
    "Fabrizio Romano": 96,
    "Gianluca Di Marzio": 95,
    "Sky Sports": 91,
    "Kicker": 90,
    "L'Équipe": 90,
    "RMC Sport": 89,
...
def strongest_source(sources: list[dict]) -> tuple[dict | None, list[dict]]:
    if not sources:
        return None, []
    ...
    ranked = sorted(
        unique.values(),
        key=lambda source: (
            SOURCE_RANK.get(source["name"], 100 if source["name"] in OFFICIAL_NAMES else 55),
            source["name"] not in WEAK_CONFIRMERS,
        ),
        reverse=True,
    )
...
def editorial_synthesis(item: dict) -> dict:
    ...
    main, confirmed = strongest_source(item.get("sources", []))
