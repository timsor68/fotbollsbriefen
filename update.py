...
UA = "Mozilla/5.0 Fotbollsbriefen/10.0-EntityFirst"
NOW = dt.datetime.now(dt.timezone.utc)
CUTOFF = NOW - dt.timedelta(days=CFG.get("max_age_days", 7))
MAX_ITEMS = min(int(CFG.get("max_items", 120)), 60)
...
def normalize_story(item: dict) -> dict:
    ...
    title = REPORTER_PREFIX.sub("", title).strip()
    title = re.sub(
    ...
    clubs = extract_clubs(f"{title}. {summary}")
    entity = item.get("entity", "")
    if entity in KNOWN_REPORTERS:
        entity = ""
    item["title"] = title
    item["summary"] = summary or f"{title}. Read the full report from the main source."
    item["entity"] = entity
    item["clubs"] = clubs
    item["club"] = clubs[0] if clubs else item.get("club", "")
    item["language"] = "en"
    return item
