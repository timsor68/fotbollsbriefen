...
def get_standings(competition_code: str) -> dict[int, dict[str, Any]]:
    ...
    goals_for, goals_against, goal_difference, home:{...}, away:{...}}}.

    football-data.orgs standings-endpoint ger TOTAL/HOME/AWAY i samma anrop
    (ingen extra API-kostnad), så vi passar på att spara hemma-/borta-split
    också — det är precis den typen av statistik en fullständig tabell ska
    ha (mål gjorda/insläppta hemma vs borta, W-D-L-split, osv)."""
    payload = fetch_football_data(f"/competitions/{competition_code}/standings")
    by_type: dict[str, dict[int, dict[str, Any]]] = {}
    total_teams = 0

    for group in payload.get("standings", []):
        gtype = group.get("type")
        if gtype not in ("TOTAL", "HOME", "AWAY"):
            continue
        rows = group.get("table", [])
        if gtype == "TOTAL":
            total_teams = max(total_teams, len(rows))
        type_table: dict[int, dict[str, Any]] = {}
        for row in rows:
            team = row.get("team", {})
            team_id = team.get("id")
            if team_id is None:
    ...
    for team_id, row in total.items():
        table[team_id] = {
            **row,
            ...
            "home": home.get(team_id),
            "away": away.get(team_id),
        }
    return table


def build_table(standings: dict[int, dict[str, Any]]) -> list[dict[str, Any]]:
    """Sorterad, komplett tabell (alla lag) för Statistik-vyn."""
    rows = list(standings.values())
    rows.sort(key=lambda r: (r.get("position") is None, r.get("position", 999)))
    return rows
...
def get_pl_injuries() -> list[dict[str, Any]]:
    """Officiell skaderapport för Premier League, ett enda anrop per körning
    (billigt — vi kör redan flera API-Football-anrop för PL varje körning
    och håller oss ändå gott och väl under 100/dygn).

    OBS: API-Football portar skadedata bakom en per-liga/säsong
    coverage-flagga. Om planen inte har täckning för PL kommer det här
    bara ge en tom lista — det märks först när vi kör mot en riktig
    nyckel, så det hanteras genom att helt enkelt inte visa panelen
    i stället för att krascha eller gissa."""
    if not APIFOOTBALL_KEY:
        return []
    season = current_season_start_year()
    try:
        payload = fetch_apifootball(
            "/injuries",
            {"league": APIFOOTBALL_PL_LEAGUE_ID, "season": season},
        )
    except Exception as exc:
        print(f"WARN PL-skaderapport misslyckades: {exc}")
        return []
    ...
    # Nyast per spelare överst, deduplicerat så samma spelare inte listas
    # flera gånger för olika matcher de missat.
    items.sort(key=lambda x: x.get("date") or "", reverse=True)
    seen: set[tuple[str | None, str | None]] = set()
    deduped: list[dict[str, Any]] = []
    for item in items:
        key = (item["team"], item["player"])
        if key in seen:
            continue
        seen.add(key)
        deduped.append(item)
...
def build_league(display_name: str, code: str) -> dict[str, Any] | None:
    ...
    return {"competition_code": code, "table": build_table(standings), "matches": output_matches}
...
def main() -> int:
    ...
    if "Premier League" in leagues:
        leagues["Premier League"]["injuries"] = get_pl_injuries()
